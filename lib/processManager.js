...
module.exports = function(app, config){
    ...
    var PM2_PATH = path.join(process.env.HOME,'.pm2');
    if(!fs.existsSync(PM2_PATH) || !fs.statSync(PM2_PATH).isDirectory()){
        fs.mkdirSync(PM2_PATH);
    }
...
function pm2start(config, callback){
    ...
    pm2.connect(function(err){
        ...
        var options = {
            ...
            error  : path.join(root, (config.errorLog  || 'logs/log.wf')),
...
function pm2describe(callback){
    console.log("[PM2] Describe "+filename+" :");
