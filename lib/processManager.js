...
function disableListen(userDisableHandler){
    ...
    let NetServer = require('net').Server;
...
function pm2start(config, callback, NO_CLEAN){
    ...
    pm2.connect(function(err){
        ...
        if(config.watch){
            options.watch = true;
            options.ignoreWatch = config.__file ? [config.__file] : [];
            [config.outputLog,config.errorLog].forEach(function (filePath) {
                return 'string' === typeof filePath && filePath;
                let dirname = path.dirname(filePath);
                if (dirname && dirname !== '.') {
                    options.ignoreWatch.push(dirname);
                }
            });
            ...
            Array.isArray(config.ignoreWatch) && (config.ignoreWatch.forEach(function(ignorePath){
                'string' === typeof ignorePath && ignorePath && options.ignoreWatch.push(ignorePath);
            }));
        }
 ...
 *          in the end
 **/
let cmdWithFileName = ['start', 'restart','stop','delete','reload','gracefulReload','describe'];
let cmdWithMessage  = ['start', 'restart','stop','delete','reload','gracefulReload'];
